    ...
    <S.FilterItemEl>
      ...
      {isActive && (
        <S.FilterItemScrollBar>
          <S.FilterItemListItems>{listItems}</S.FilterItemListItems>
        </S.FilterItemScrollBar>
      )}
    </S.FilterItemEl>
  )
    ...
    <S.MainCenterblock>
      ...
      {isFilterVisible && (
        <S.CenterblockFilter>
          ...
          <FilterItem
            title="исполнителю"
            listItems={listPerformer}
            onShow={clbSetActiveIndex(1)}
            isActive={activeIndex === 1}
          />
          <FilterItem
            title="жанру"
            listItems={listGenre}
            onShow={clbSetActiveIndex(2)}
            isActive={activeIndex === 2}
          />
          <FilterItem
            title="году выпуска"
            listItems={listYear}
            onShow={clbSetActiveIndex(3)}
      ...
      <S.CenterBlockContent>
        ...
        {LoadTracksError ? (
          `Не удалось загрузить плейлист,попробуйте позже:${LoadTracksError}`
        ) : (
          <TrackList
            unvisible={isSkeleton}
            arrOfTracks={arrOfTracks}
